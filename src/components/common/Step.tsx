...
function StepCircle({ stepNumber, text, isActive, isCompleted }: StepCircleProps) {
  return (
    <div className={css({ ...stepWrapperStyles })}>
      <div
        className={cx(stepCircleStyles, isActive || isCompleted ? activeBackgroundStyles : inactiveBackgroundStyles)}>
        {isCompleted ? (
      ...
      <span className={cx(stepTextStyles, isActive && activeTextStyles, !isActive && inactiveTextStyles)}>{text}</span>
    </div>
  );
}
...
export function StepConnector({ isCompleted = false }: StepConnectorProps) {
  return <div className={cx(connectorStyles, isCompleted ? activeBackgroundStyles : inactiveBackgroundStyles)} />;
}
